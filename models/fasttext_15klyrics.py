...
# Train word2vec
path = get_tmpfile("fifteenklyricsft_w5ns5_notcleaned_nopad.model")
cores = multiprocessing.cpu_count()
ftmodel = FastText(min_count=1,
                   window=5,
                   size=300,
                   sample=6e-5,
                   alpha=0.03,
...
t = time()
ftmodel.build_vocab(ngrams_sent, progress_per=1000)
print('Time to build vocab: {} mins'.format(round((time() - t) / 60, 2)))

phrasest = time()
ftmodel.train(ngrams_sent, total_examples=ftmodel.corpus_count,
              epochs=50, report_delay=1)
print('Time to train the model: {} mins'.format(round((time() - t) / 60, 2)))
ftmodel.save('fifteenklyricsft_w5ns5_notcleaned_nopad.model')
